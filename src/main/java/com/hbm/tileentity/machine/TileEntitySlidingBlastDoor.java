    ...
    public void handleNewState(DoorState newState) {
        if (this.state != newState) {
            if (this.state == DoorState.CLOSED && newState == DoorState.OPENING) {
                if (audio == null) {
                    audio = MainRegistry.proxy.getLoopedSoundStartStop(world, HBMSoundEvents.qe_sliding_opening, null,
							HBMSoundEvents.qe_sliding_opened, SoundCategory.BLOCKS, pos.getX(), pos.getY(), pos.getZ(), 2, 1);
                    audio.startSound();
                }
            }
            if (this.state == DoorState.OPEN && newState == DoorState.CLOSING) {
                if (audio == null) {
                    audio = MainRegistry.proxy.getLoopedSoundStartStop(world, HBMSoundEvents.qe_sliding_opening, null,
							HBMSoundEvents.qe_sliding_shut, SoundCategory.BLOCKS, pos.getX(), pos.getY(), pos.getZ(), 2, 1);
                    audio.startSound();
                }
            }
