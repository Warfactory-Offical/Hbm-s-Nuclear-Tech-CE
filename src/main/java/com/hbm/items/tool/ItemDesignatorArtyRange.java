...
public class ItemDesignatorArtyRange extends ItemBase {
    ...
    if (itemStack.getTagCompound() == null) {
      ...
      tooltip.add(
          TextFormatting.YELLOW
              + "Linked to "
              + itemStack.getTagCompound().getInteger("x")
              + ", "
              + itemStack.getTagCompound().getInteger("y")
              + ", "
              + itemStack.getTagCompound().getInteger("z"));
    }
  }
      ...
      int[] pos =
          ((BlockDummyable) b)
              .findCore(
                  ...
                  clickPos.getX(),
                  clickPos.getY(),
                  clickPos.getZ()); // TODO: Make this run on blockPos
  ...
  public @NotNull ActionResult<ItemStack> onItemRightClick(
      @NotNull World world, EntityPlayer player, @NotNull EnumHand hand) {
