...
public class EntityPlasmaBeam extends Entity implements IProjectile {
    ...
    private boolean surviveImmolation(EntityPlayer player) {                             //gun_revolver_pip
    	if(player instanceof EntityPlayerMP && Library.hasInventoryItem(player.inventory, DummyTexs.nothing) && Library.hasInventoryItem(player.inventory, Foods.bottle_rad) && Library.hasInventoryItem(player.inventory, ModItems.geiger_counter)) {
