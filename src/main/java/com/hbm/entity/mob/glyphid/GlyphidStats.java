...
public abstract class GlyphidStats {
    ...
    public record StatBundle(double health, double speed, double damage, @Deprecated float divisor,
            ...
            public StatBundle(double health, double speed, double damage, float divisor, float damageThreshold, float thresholdMultForArmor, float resistanceMult) {
                this.health = health;
                this.speed = speed;
                this.damage = damage;
                this.divisor = divisor;
                this.damageThreshold = damageThreshold;
                this.thresholdMultForArmor = thresholdMultForArmor;
    ...
    public static class GlyphidStats70K extends GlyphidStats {
        ...
        public boolean handleAttack(EntityGlyphid glyphid, DamageSource source, float amount) {
            ...
            if(!source.isDamageAbsolute() && !source.isUnblockable() && !glyphid.world.isRemote && !source.isFireDamage() && !source.getDamageType().equals(ModDamageSource.s_cryolator)) {
                byte armor = glyphid.getDataManager().get(EntityGlyphid.ARMOR);
            ...
            if(source.isFireDamage()) {
                ...
                amount *= (float) (glyphid.getScale() < 1.25 ? 1.5 : glyphid.getScale() < 1.3 ? 0.8 : 0.5);
            ...
            if(glyphid.isPotionActive(HbmPotion.phosphorus)){
                amount *= 1.5F;
            }
    ...
    public static class GlyphidStatsNT extends GlyphidStats {
        ...
        public boolean handleAttack(EntityGlyphid glyphid, DamageSource source, float amount) {
            ...
            if((source == ModDamageSource.acid || ModDamageSource.s_acid.equals(source.getDamageType())) && source.getTrueSource() instanceof EntityGlyphid) return false;
            return glyphid.attackSuperclass(source, amount);
        }
    }
}
